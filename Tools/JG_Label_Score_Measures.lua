...
--   track ("Partiturtakte"), labelled with the *printed* score measure number.
...
--
--   Items are time-base "beats (position + length)" so they follow tempo-map
--   edits. The label track is pinned to the top, height-locked and its items
--   are locked, so it can't be moved/resized/edited by accident.
--
--   Workflow: set markers -> run. Change something -> adjust markers -> run
--   again (old labels are replaced automatically).

-- ── Config ───────────────────────────────────────────────────────────────
local LABEL_TRACK_NAME   = "Partiturtakte"
...
local LABEL_TRACK_HEIGHT = 30                -- px (only used when LOCK_HEIGHT)
local proj = 0
