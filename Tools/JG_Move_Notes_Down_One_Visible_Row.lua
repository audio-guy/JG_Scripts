-- @description Move notes down one visible row (drum-map / custom note order aware)
-- @author Julius Gass
-- @about
--   MIDI editor: moves every selected note DOWN by one VISIBLE ROW instead of by
--   one semitone.
--
--   * In a normally pitch-sorted map this is identical to one semitone down
--     (like the stock action 40178).
...
--     jumps to the next visible row BELOW in display order, even when that row
...
--   that order (read-only) and steps one entry toward the bottom. It falls back
--   to a plain one-semitone move when the track has no custom order, or for a
...
--   the bottommost visible row stay put (no wrap-around).
--
--   Pairs with: JG_Move_Notes_Up_One_Visible_Row.lua

local r = reaper
local DIR = -1 -- -1 = down (toward the bottom of the editor)
...
local function run()
  ...
  r.MIDI_Sort(take)
  r.Undo_EndBlock2(0, "Move notes down one visible row", -1)
end
